...
type (
	EventManager interface {
		...
		Dispatch(event any)
		DispatchAsync(event any, parallel bool)
		fmt.Stringer
	}
...
func (e *eventManager) Dispatch(event any) {
	e.lock.RLock()
	defer e.lock.RUnlock()
	...
	e.run(eventName, eventValue, eventPointerValue)
	return
}

func (e *eventManager) run(eventName string, eventValue, eventPointerValue reflect.Value) {
...
func (e *eventManager) DispatchAsync(event any, parallel bool) {
	go func() {
		e.lock.RLock()
		defer e.lock.RUnlock()
