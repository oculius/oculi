...
func NewRoute(r *echo.Route, h any) Route {
	if h != nil {
		r.Name = handlerName(h)
	}
	return Route{r}
}
...
func (g *group) RGroup(prefix string, middleware ...MiddlewareFunc) RouteGroup {
	return NewRouteGroup(g.Group.Group(prefix, mote(middleware)...))
}

func (g *group) Bundle(prefix string, bundler func(RouteGroup), middleware ...MiddlewareFunc) RouteGroup {
	rg := NewRouteGroup(g.Group.Group(prefix, mote(middleware)...))
	bundler(rg)
	return rg
}
...
func (g *group) Add(method string, path string, h HandlerFunc, m ...MiddlewareFunc) Route {
	return NewRoute(g.Group.Add(method, path, H(h), mote(m)...), h)
}

func (g *group) FileFS(path string, file string, filesystem fs.FS, m ...MiddlewareFunc) Route {
	return NewRoute(g.Group.FileFS(path, file, filesystem, mote(m)...), g.FileFS)
}
